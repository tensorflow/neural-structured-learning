...
r"""Train a model using parameter generators for the decoder.

This script trains a model that uses an autoencoder style architecture to
generate obfuscated embeddings close to the clean ones. The model in question
relies on parameter generators to construct the parameters of the decoder. These
parameter generators derive a context vector via a context oracle, which is
provided the obfuscation label as input. The context vectors provided by the
oracle are then provided to the parameter generator.
...
_LR_DECAY_FACTOR = flags.DEFINE_float(
    ...
    'Factor with which to decay learning rate per decay epoch.')
...
_WEIGHT_DECAY = flags.DEFINE_float('weight_decay', 1e-6, 'Weight decay.')
...
_LATENT_DIM = flags.DEFINE_integer('latent_dim', 128, 'Latent dimension.')
_CONTEXT_DIM = flags.DEFINE_integer('context_dim', 32, 'Context dimension.')

_CONTEXT_MLP_SIZES = flags.DEFINE_list(
    'context_mlp_sizes', ['256', '128'], 'Context mlp sizes.')

_PARAM_GENERATOR_MLP_SIZES = flags.DEFINE_list(
    'param_generator_mlp_sizes', ['128'],
    'Param generator MLP sizes')
...
_EPOCH_LIM = flags.DEFINE_integer(
    ...
    'Set -1 to assign a constant weight to the loss contribution.')
...
_EMBED_LOSS_WEIGHT = flags.DEFINE_float(
    'embed_loss_weight', 0.2, 'Factor with which to multiply embedding loss.')

_EMBED_RECON_LOSS = flags.DEFINE_string('embed_recon_loss', 'Contrastive',
                                        'Loss used to match the generated'
                                        'embeddings.')
...
_MODEL_TYPE = flags.DEFINE_enum(
    'model_type', 'vitb32',
    ['resnet50', 'resnet101', 'resnet152', 'bit-m152x4', 'vitb32'],
...
def train_and_eval_model(
  ...
  with strategy.scope():
    loss_names = [
        ...
        'clf_train_embed_loss', 'clf_train_context_loss'
    ]
    ...
    def step_fn(inputs):
      ...
      with tf.GradientTape() as tape:
        ...
        crossentropy_loss = losses_lib.weighted_crossentropy_loss(
            labels, real_logits, generated_logits, loss_ramp_weight
        )
...
def main(argv: Sequence[str]) -> None:
  ...
  with strategy.scope():
    ...
    context_mlp_sizes = [
        int(_CONTEXT_MLP_SIZES.value[i])
        for i in range(len(_CONTEXT_MLP_SIZES.value))
    ]

    param_generator_mlp_sizes = [
        int(_PARAM_GENERATOR_MLP_SIZES.value[i])
        for i in range(len(_PARAM_GENERATOR_MLP_SIZES.value))
    ]

    encoder = extended_model.ParameterGenerationEmbeddingMapper(
        encoder_decoder_mlp_sizes=mlp_sizes,
        param_generator_mlp_sizes=param_generator_mlp_sizes,
        context_mlp_sizes=context_mlp_sizes,
        embed_dim=model_config.embed_dim,
        latent_dim=_LATENT_DIM.value,
        context_dim=_CONTEXT_DIM.value,
        weight_decay=_WEIGHT_DECAY.value,
        num_contexts=len(_OBFUSCATIONS_TRAIN.value)-1,
        generation=True,
    )

    model = extended_model.FeatureExtractor(
        model_link=model_config.model_link,
        bypass_base_model=True,  # Set to True due to a compatibility issue.
        encoder=encoder,
