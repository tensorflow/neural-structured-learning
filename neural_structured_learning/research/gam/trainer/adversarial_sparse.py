...
def get_loss_vat(inputs,
  ...
  r_vadv = generate_virtual_adversarial_perturbation(
      inputs, predictions, model, placeholders, mask, predictions_var_scope,
      is_train=is_train)
...
def entropy_y_x(logits, mask):
  ...
  Args:
    ...
    mask: A boolean Tensor specifying which samples to use in the calculation
      of the entropy.
