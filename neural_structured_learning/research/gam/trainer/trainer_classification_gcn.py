...
from gam.trainer.adversarial_sparse import entropy_y_x
from gam.trainer.adversarial_sparse import get_loss_vat
...
class TrainerClassificationGCN(Trainer):
  ...
  def __init__(self,
    ...
    # Create accuracy.
    accuracy = tf.equal(
      tf.argmax(normalized_predictions_batch, 1),
      input_labels)
  ...
  def predict(self, session, indices, is_train):
    ...
    predictions = session.run(
        self.normalized_predictions_batch,
        feed_dict=feed_dict)
    return predictions
